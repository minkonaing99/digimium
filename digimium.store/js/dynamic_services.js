...
class DynamicServicesRenderer {
    ...
    return `
            <article class="service-card">
                <div class="service-image">
                    <img src="${this.getImageUrl(service.photo_url)}" alt="${service.name}" loading="lazy" onerror="this.onerror=null;this.src='images/services/default.jpg';">
                </div>
                <h3>${service.name}</h3>
                ${badges}
  ...
  renderBadges(badges) {
    ...
    const items = badges
      ...
      .map(
        (badge) =>
          `<span style="display:inline-flex;align-items:center;padding:0.22rem 0.6rem;border-radius:999px;border:1px solid rgba(77,178,221,.28);background:rgba(77,178,221,.12);color:#bfe9ff;font-size:.72rem;font-weight:600;letter-spacing:.02em;">${badge}</span>`
      )
      .join("");

    return `<div style="display:flex;flex-wrap:wrap;gap:.45rem;margin:-0.35rem 0 1rem;">${items}</div>`;
  }
