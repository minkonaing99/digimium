...
try {
    ...
    $errors = [];
    if (empty($data['sale_product']))                                        $errors['sale_product']   = 'Product is required';
    if (empty($data['customer']))                                            $errors['customer']       = 'Customer is required';
    if (empty($data['purchased_date']))                                      $errors['purchased_date'] = 'Purchase date is required';
    if (!isset($data['price'])  || !is_numeric($data['price'])  || $data['price']  < 0) $errors['price']  = 'Valid price is required';
    if (!isset($data['profit']) || !is_numeric($data['profit']))             $errors['profit']         = 'Valid profit is required';

    if (!empty($errors)) {
        http_response_code(422);
        echo json_encode(['success' => false, 'errors' => $errors], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
        exit;
    }
    ...
    if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $purchased_date))                    $errors['purchased_date'] = 'Invalid purchase date format (YYYY-MM-DD)';
    if ($expired_date && !preg_match('/^\d{4}-\d{2}-\d{2}$/', $expired_date))     $errors['expired_date']   = 'Invalid expired date format (YYYY-MM-DD)';
    ...
    error_log('ws_sale_insertion.php error: ' . $e->getMessage());
