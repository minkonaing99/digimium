...
$configuredStorefrontUrl = Config::get('DIGIMIUM_STOREFRONT_PUBLIC_URL');
if ($configuredStorefrontUrl) {
    $storefrontBaseUrl = rtrim($configuredStorefrontUrl, '/');
} else {
    $scheme = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
    $host = (string)($_SERVER['HTTP_HOST'] ?? '');
    $scriptDir = str_replace('\\', '/', dirname((string)($_SERVER['SCRIPT_NAME'] ?? '')));
    $localPath = preg_replace('#/admin\.digimium\.store$#', '/digimium.store', $scriptDir) ?: '/digimium.store';

    if ($host !== '' && str_starts_with($host, 'admin.')) {
        $storefrontBaseUrl = $scheme . '://' . substr($host, 6);
    } elseif ($host !== '') {
        $storefrontBaseUrl = $scheme . '://' . $host . rtrim($localPath, '/');
    } else {
        $storefrontBaseUrl = rtrim($localPath, '/');
    }
}
?>
...
<body>
    ...
    <script>
        window.PRODUCT_SHOWCASE_CONFIG = {
            storefrontBaseUrl: <?php echo json_encode($storefrontBaseUrl, JSON_UNESCAPED_SLASHES); ?>
        };
    </script>
